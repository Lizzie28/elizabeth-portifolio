...
export function BlogSection({ data = blogData }: BlogSectionProps) {
  // Use a custom empty state if no posts exist
  const posts = data?.posts ?? blogData?.posts ?? []
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      {hasPosts ? (
        ...
        // --- Empty state for no blog posts ---
        <p className="text-muted-foreground text-sm md:text-base">
          I’m currently focused on learning and building projects. Blog posts will appear here once I start sharing my experiences.
        </p>
      )}
    </div>
  )
